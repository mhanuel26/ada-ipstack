...
is
   ...
   type State_Kind is
     (SS_FREE, SS_READY, SS_ACCEPTED, SS_RECEIVED, SS_CLOSING);
   ...
   --  type Socket_State_Array is array (Valid_SS_Id) of Socket_State;
   --  pragma Suppress_Initialization (Socket_State);

   type Socket_Server_Cbks is record
      --  Callbacks
      Socket_Create_Cb      : Callbacks.CBK_Id;
      Socket_Receive_Cb     : Callbacks.CBK_Id;
   end record;

   subtype SS_Id is AIP.EID range 0 .. 3;
   subtype Valid_SS_Id is SS_Id range SS_Id'First + 1 .. SS_Id'Last;
   NOSS : constant SS_Id := SS_Id'First;
   ...
   --     type Client_Ptr is access all Connection;
   --
   ...
   type Connections_Factory_ptr is access all Connections_Factory;
   ...
   procedure Clear (Client : in out Connection'Class);
...
private
   ...
   type Connection
     (Input_Size  : Buffer_Length;
      ...
      record
         --  the lwip port additions
         --           State_Pool       : Socket_State_Access;
         Sid              : SS_Id;
         Client_Address   : AIP.IPaddrs.IPaddr;
         Client_Port      : AIP.U16_T;
         Err              : AIP.Err_T;
