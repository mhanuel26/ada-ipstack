...
--  with STM32.Calendar;           use STM32.Calendar;
...
with AIP.TCP;
with System;
with RAW_LwIp_Server.API_Process;
with AIP.IO;

package body RAW_LwIp_Server with
  Refined_State => (State => (RAW_LwIp_Server.API_Process.State))
is
...
--         Free (Client.Transport);
   ...
   procedure Connect_Error
             (Client : in out Connection --   ; --   Error  : Error_Type
             )  is
   begin
      null;
   end Connect_Error;
...
--            Append
--            (Client.Listener.Postponed,.all'Unchecked_Access;
--               Client'Unchecked_Access,
...
--        );
   ...
   begin
      pragma Unreferenced (Client, Data, Last);
      null;
--        if Client.Transport = null then
...
--        end if;
   ...
   procedure Read
             (Client  : in out Connection --   ; --   Factory : in out Connections_Factory'Class
             )  is
...
--           raise Connection_Error;
   ...
   --  pointer is not modified but in order to maintain same callback structure
   ...
   begin
      ...
      Received (Client    =>  Client.all,
                Data      =>  DATA,
                Pointer   =>  Pointer);
      Err := AIP.NOERR;
   end Do_Receive;
   pragma Warnings (On, "formal parameter * is not modified");
...
--        pragma Unreferenced (Buffer);
   ...
   begin
      ...
      while Err = AIP.NOERR
        and then Ss.Buf /= AIP.Buffers.NOBUF
        and then AIP.Buffers.Buffer_Len (Ss.Buf) <= AIP.TCP.TCP_Sndbuf (Pcb)
      loop
   ...
   begin
      size := AIP.U16_T (Data'Last - Data'First + 1);
      ...
      if p /= AIP.Buffers.NOBUF then
         ...
         declare
            Ss : Socket_State;
            for Ss'Address use SSP (Client.Sid)'Address;
         begin
            Ss.Buf := p;
            Socket_Send (Ss.Pcb, Ss);
            Last := Data'Last;
         end;
      else
         Last := Client.Written.First_Written - 1;  --  Not sure about this
         --  since original code always add 1 to Next
         Client.Err := ERR_MEM;
      end if;
   end Send_Server;
...
--        end if;
   ...
   begin
      ...
      --  TODO: should port this by setting client pointer appropiately
      --  which at the end should call Socket_Close
--        Close (Client.Socket);
      Client.Session := Session_Down;
      Client.Failed  := False;
      Client.Shutdown_Request := False;
   ...
   procedure Unblock_Send
     (Listener : in out Connections_Server;
      ...
      Buffer : Output_Buffer renames Client.Written;
   begin
            --  This is the pure callback implementation - No Task
--           declare
--              Block : Boolean;
--           begin
--              Write (Client, Block);
...
--                 end if;
         ...
         --  At the end qwe update the Blocked var
         Blocked := Buffer.First_Written = Buffer.Free_To_Write;
      end if;
   end Write;
