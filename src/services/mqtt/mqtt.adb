...
with Connection_State_Machine.Big_Endian.Unsigneds;
...
package body MQTT with
  Refined_State => (State => (MQTT.Clients.State))
is
...
--        Finalize (Connection (Pier));
   ...
   function Get_Topic (Topic : Topic_Item) return String;
   ...
   function Header_Image (Header : Stream_Element) return String;
   ...
   begin
      pragma Unreferenced (Client, Clean, Will_Topic, Will_Message,
                          Will_QoS, Will_Retain, User_Name, Password, Keep_Alive);
...
--        Shutdown (Pier);
   ...
   begin
      --  The procedure is called when a successful connection acknowledge CONNACK is received
      null;
   ...
   begin
      pragma Unreferenced (Topic, Message, Duplicate, Retain);
      case Packet.QoS is
         when At_Most_Once =>
            null;
         when At_Least_Once =>
   ...
   begin
      pragma Unreferenced (Topics_Number);
...
--                 end if;
            ...
            when MQTT_Connect_Return =>
               ...
               if Data (Pointer - 1) = 0 then
                  Pier.State_Pool.Kind := MS_CONNECTED;
            ...
            when MQTT_String_Length_LSB =>
               ...
               if Pier.Length < Pier.Count then
                  ...
                  (Data_Error'Identity,
                     ...
                     &  " has length "
                     &  Stream_Element_Count'Image (Pier.Count)
                     &  " exceeding remaining payload data "
                     &  Stream_Element_Count'Image (Pier.Length)
                  ));
               end if;
               declare
                  This : MQTT_String_Ptr :=
   ...
   procedure Init with
      Refined_Global => (Output => (Clients.State))
   is
   begin
...
--     pragma Warnings (Off, "variable * is *");
   procedure Send
             (Pier : in out MQTT_Pier;
              Data : Stream_Element_Array
             ) is
...
--     pragma Warnings (On, "variable * is *");
   ...
   with
     Refined_Global => (In_Out => (Clients.State))
   is
   begin
      Err := AIP.NOERR;
      Clients.Mqtt_Client_Connect
        (Pier              =>  Pier,
         Err               =>  Pier.Err);
      if Pier.Err /= AIP.NOERR then
         Err := Pier.Err;
      end if;
   end Connect;

   function Is_Connected (Pier : in out MQTT_Pier) return Boolean is
   begin
      return Pier.State_Pool.Kind = TCP_CONNECTED or
        Pier.State_Pool.Kind = MS_CONNECTED;
   end Is_Connected;
   ...
   procedure Disconnect
      ...
      for Ms'Address use Clients.MSP (Pier.Sid)'Address;
   begin
      Clients.Mqtt_Client_Disconnect
         (Pier         =>  Pier,
          Err          =>  Pier.Err);
      Err := Pier.Err;
   end Disconnect;
   ...
   procedure Send_Connect_Accepted
      ...
      Data : Stream_Element_Array (1 .. 4)
        := (Stream_Element (CONNACK_Request),
            Stream_Element (2),
            Stream_Element (0),
            Stream_Element (0));
   begin
      if Session_Present then
         Data (3) :=  Stream_Element (1);
      end if;
      Send (MQTT_Pier'Class (Pier), Data);
   ...
   procedure Send_Disconnect (Pier : in out MQTT_Pier) is
      --  For some reason the original version don't compile correctly here
      Data : constant Stream_Element_Array (1 .. 2)
        := (Stream_Element (DISCONNECT_Request), Stream_Element (0));
   begin
      --  Inform TCP Stack we want to send FIN with Disconnect Request.
      Pier.State_Pool.Kind := MS_CLOSING;
      Send (MQTT_Pier'Class (Pier), Data);
   end Send_Disconnect;

   procedure Send_Ping (Pier : in out MQTT_Pier) is
      Data : constant Stream_Element_Array (1 .. 2)
        := (Stream_Element (PINGREQ_Request), Stream_Element (0));
   begin
      Send (MQTT_Pier'Class (Pier), Data);
   end Send_Ping;
   ...
   procedure Send_Ping_Response (Pier : in out MQTT_Pier) is
      Data : constant Stream_Element_Array (1 .. 2)
        := (Stream_Element (PINGRESP_Request), Stream_Element (0));
   begin
      Send (MQTT_Pier'Class (Pier), Data);
...
--        );
   ...
   procedure Set_Client_Cbs
     (Pier         : in out MQTT_Pier;
      Connect_Cb   : MQTT_Hook;
      Receive_Cb   : MQTT_Hook)
   is
   begin
      Pier.App_Cbs.App_Connect_Cb := RAW_MQTT_Callbacks.To_CBID (Connect_Cb);
      Pier.App_Cbs.App_Receive_Cb := RAW_MQTT_Callbacks.To_CBID (Receive_Cb);
   end Set_Client_Cbs;
