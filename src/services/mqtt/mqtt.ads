...
with AIP.PCBs;
with AIP.Callbacks;
with AIP.Time_Types;       use AIP.Time_Types;
with AIP.Timers;           use AIP.Timers;
with AIP.IPaddrs;
with RAW_MQTT_Callbacks;   use RAW_MQTT_Callbacks;
...
--
   ...
   type Mqtt_Conn_State_Kind is
     (MS_FREE, MS_READY, MS_ACCEPTED, MS_RECEIVED, MS_CONNECTING, MS_CONNECTED, MS_CLOSING,
      TCP_CONNECTED, TCP_CONNECTING, TCP_DISCONNECTED);
   type Mqtt_Client_Cbks is record
      --  Callbacks
      App_Connect_Cb     : Callbacks.CBK_Id;
      App_Receive_Cb     : Callbacks.CBK_Id;
   end record;
   type Mqtt_Client_Cbks_Ptr is access Mqtt_Client_Cbks;
   subtype Valid_Mqtt_Tmr_Id is Valid_Tmr_Id range Valid_Tmr_Id'Range;
   type Mqtt_Conn_State is record
      Kind : Mqtt_Conn_State_Kind;
      Pcb  : AIP.PCBs.PCB_Id;
      Buf  : AIP.Buffers.Buffer_Id;
      --  Timers and timeouts
      Tmr_Id           : Valid_Mqtt_Tmr_Id;
      Keep_Alive       : AIP.U16_T;
      Cyclic_Tick      : AIP.U16_T;
      Server_Watchdog  : AIP.U16_T;
      App_Client_Cbs   : Mqtt_Client_Cbks;
      Err  : AIP.Err_T;
   end record;
   type Mqtt_Conn_State_Access is access all Mqtt_Conn_State;
   pragma Suppress_Initialization (Mqtt_Conn_State);
   subtype MS_Id is AIP.EID range 0 .. 3;
   subtype Valid_MS_Id is MS_Id range MS_Id'First + 1 .. MS_Id'Last;
   type Mqtt_Conn_State_Array is array (Valid_MS_Id) of aliased Mqtt_Conn_State;
   --  type Mqtt_Conn_State_Array_Access is access all Mqtt_Conn_State_Array;

   NOMS : constant MS_Id := MS_Id'First;

--     MSP : Mqtt_Conn_State_Array; -- Mqtt_Conn_State Pool
   --  Current MQTT time counter (Time_Types.Hz ticks per second)

   MQTT_Tick_Interval : constant := Time_Types.Hz;

   MQTT_CYCLIC_TIMER_INTERVAL : constant := 5;

   MQTT_CONNECT_TIMOUT : constant := 100;

   TCP_CONNECT_TIMOUT : constant := 10;
...
--  sending.
--
   type Connection is abstract new Object.Entity with private;
...
--
   ...
   function Is_Connected (Pier : in out MQTT_Pier) return Boolean;
...
--
--  The default implementation  confirms delivery for the packages if the
...
--
--  Set_Client_Ip --  Set the Client Ip Address to connect with
--
...
------------------------------------------------------------------------
   ...
   type MQTT_Pier_Ptr is access all MQTT_Pier;
   type MQTT_Pier_Array_Ptr is array (Valid_MS_Id) of MQTT_Pier_Ptr;
