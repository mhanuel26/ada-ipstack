      ...
      begin
         ...
         Ethernet_MAC_Periph.MACCR :=
           (CSTF => True,
            ...
            LM   => False,
            DM   => True,
            --  IPv4 checksum offload activated
            IPCO => True,
            RD   => False,
            APCS => True,
            BL   => 2#10#,
            ...
            RE   => False,
            others => <>);
         --  Pass to App opnly those frames that pass the filter
         Ethernet_MAC_Periph.MACFFR :=
           (RA => True, others => <>);
         --  Ethernet_MAC_Periph.MACFFR :=
         --    (RA => True, PM => True, others => <>);
         --  Set the Mac Address Value for Filtering
         --  Ethernet_MAC_Periph.MACA0HR :=
         --    (MO => True, MACA0H => 16#0105#, others => <>);
         --  Ethernet_MAC_Periph.MACA0LR := 16#05E18100#;
         ...
         --  Added to verify issue
         --  Ethernet_DMA_Periph.DMAOMR.RSF := False;
         --  Ethernet_DMA_Periph.DMAOMR.FUGF := True;
         --  Ethernet_DMA_Periph.DMAOMR.RTC := 2#01#;
         --  added
         Ethernet_DMA_Periph.DMAOMR.SR := True;
