      ...
      begin
         ...
         case Proto is
            ...
            when IPH.IP_Proto_ICMP =>
               ...
               Icmp_Tbytes := Icmp_Tbytes + Net.Uint64 (Buffers.Buffer_Tlen (Buf));

            when others =>
               --  Discard IP packet with unknown protocol
               --  This seems to mess the state machine
               --  for now disable it
--                 ICMP.ICMP_Reject
--                   (Buf,
...
--                    Code   => ICMPH.ICMP_Unreach_Code_Proto_Unreachable);
   ...
   begin
      ...
      IpChSumEn := NIF.Offload (Netif, NIF.IP_CS);
      IpHdrLen := AIP.U16_T (IPH.IPH_IHL (Ihdr)) * 4;
      BufLen := Buffers.Buffer_Tlen (Buf);

      if not IpChSumEn then
         ChSumRes := Checksum.Sum (Buf => Buf, Length => AIP.U16_T (IPH.IPH_IHL (Ihdr)) * 4);
         if ChSumRes /= 16#Ffff# then
            Err := AIP.ERR_USE;
         end if;
      end if;

      if False
        or else BufLen < IPH.IP_Header_Size / 8
        or else BufLen < IpHdrLen
      ...
      if AIP.No (Err) then
         if Local then
            ...
            Proto := IPH.IPH_Protocol (Ihdr);
            Dispatch_Upper (Proto, Buf, Netif, Err);
