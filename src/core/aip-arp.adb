...
is
   ...
   begin
      ...
      Timers.TID_Alloc (Timers.TIMER_EVT_ETHARPTMR, ARP_Timer'Access);
            ...
            case ARPH.ARPH_Operation (Ahdr) is
               when ARPH.ARP_Op_Request =>
                  if Local then
                     ...
                     EtherH.Set_EtherH_Src_MAC_Address (Ehdr, Netif_MAC_Addr);
                     ...
                     ARPH.Set_ARPH_Src_Eth_Address (Ahdr, Netif_MAC_Addr);
   ...
   procedure ARP_Flush (All_Entries : Boolean) is
      Now        : Time_Types.Time;
      AEID       : Any_ARP_Entry_Id;
   ...
   procedure ARP_Timer (Id : Integer) with
     Refined_Global =>
       (In_Out => (ARP_Active_List, ARP_Free_List, ARP_Table, Buffers.State))
   is
   begin
