...
is
   ...
   begin
      ...
      Seg_Len := Seg_Len + AIP.M32_T (TCPH.TCPH_Syn (Thdr));
      Seg_Len := Seg_Len + AIP.M32_T (TCPH.TCPH_Fin (Thdr));
      Seg_Len := Seg_Len - 4 * AIP.M32_T (TCPH.TCPH_Data_Offset (Thdr));
...
--                     + AIP.M32_T (TCPH.TCPH_Fin (Thdr));
      ...
      loop
         ...
         if AIP.No (Err) then
            ...
            if D_Buf /= Buf then
               ...
               --  as needed.
               --  I force this to ACK True during MQTT client test
               --  But now it's generating extra Ack in server mode
               D_Buf := Buffers.NOBUF;
   ...
   begin
      ...
      case TPCBs (PCB).State is
         ...
         when Fin_Wait_1 =>
            --  Its possible to go from Fin_Wait_1 to Time_Wait
            --  as stated here http://www.cs.northwestern.edu/~agupta
            --  /cs340/project2/TCPIP_State_Transition_Diagram.pdf
            pragma Assert (State = Fin_Wait_2
                           or else State = Closing
                           or else State = Time_Wait);
            null;
   ...
   begin
      --  Purge Send Queue
      loop
         Packet := Buffers.Head_Packet (TPCBs (PCB).Send_Queue);
         exit when Packet = Buffers.NOBUF;
         Buffers.Remove_Packet
           (Buffers.Transport, TPCBs (PCB).Send_Queue, Packet);
      ...
      if Seq_Lt (Seg.Ack, TPCBs (PCB).SND_UNA) then
         ...
         loop
            ...
            if TCPH.TCPH_Fin (Thdr) = 1 then
               case TPCBs (PCB).State is
                  when Fin_Wait_1 =>
                     --  We received the Fin-Ack so move into TIME-WAIT
                     Set_State (PCB, Time_Wait);
                     --  sort of TCP PCB PURGE ...
                     TPCBs (PCB).Send_Queue := Buffers.Empty_Packet_Queue;
                     TPCBs (PCB).Unack_Queue := Buffers.Empty_Packet_Queue;
                     TCP_Purge (PCB);
                     TPCBs (PCB).IRS     := Seg.Seq + 1;
                     TPCBs (PCB).RCV_NXT := TPCBs (PCB).IRS;
                     --  Ack Now
                     TCP_Output (PCB => PCB, Ack_Now => True);
                     pragma Warnings (Off, "unused assignment to ""Err""");
                     --  Discard error from user code
...
--                       TCP_Free (PCB);
               ...
               if TPCBs (PCB).State = Fin_Wait_1 then
         ...
         --  If nothing remains on the Unack_Que ue, stop retransmit timer,
         --  else reset it.
         ...
         when Syn_Sent =>
            ...
            if not Discard then
               ...
               if TCPH.TCPH_Syn (Seg.Thdr) = 1 then
                  ...
                  if TPCBs (PCB).SND_UNA > TPCBs (PCB).ISS
                    and then TCPH.TCPH_Ack (Seg.Thdr) = 1
                  then
                     Set_State (PCB, Established);
                     TCP_Purge (PCB);
                     --  TPCBs (PCB).Send_Queue := Buffers.Empty_Packet_Queue;
                     --  TPCBs (PCB).Unack_Queue := Buffers.Empty_Packet_Queue;
                     TPCBs (PCB).RCV_NXT := TPCBs (PCB).RCV_NXT - 1;
                     declare
                        Packet : Buffers.Buffer_Id;
                     begin
...
--                          Packet := Buffers.Head_Packet (TPCBs (PCB).Unack_Queue);
                        ...
                        --  Discard error from user code
                        TCP_Event
                          (Ev   => TCP_Event_T'(Kind => TCP_EVENT_CONNECT,
                                                Len  => 0,
                                                Buf  => Buffers.NOBUF,
                                                Addr => IPaddrs.IP_ADDR_ANY,
                                                Port => PCBs.NOPORT,
                                                Err  => AIP.NOERR),
                           PCB  => PCB,
                           Cbid => TPCBs (PCB).Callbacks (TCP_EVENT_CONNECT),
                           Err  => Err);
                        pragma Warnings (On, "unused assignment to ""Err""");
                        TCP_Output (PCB => PCB, Ack_Now => True);
                        TCP_Purge (PCB);
            ...
            if not Discard then
               case TPCBs (PCB).State is
                  ...
                  when Fin_Wait_1 =>
                     ...
                     --  Here we go to Process the Ack
                     --  which also checks for FIN and sends ACK Now if found
                     Process_Ack (PCB, Seg);

                  when Established | Fin_Wait_2 =>
                     ...
                     --  Added this condition of FIN Check since we are
                     --  forcing Ack (Ack_Now) inside Deliver Segment
                     --  this way we don't ACK the FIN, which actually need FIN-ACK
                     --  which is done later on.

--                       if New_Data_Len > 0 and TCPH.TCPH_Fin (Seg.Thdr) /= 1
               ...
               if TCPH.TCPH_Fin (Seg.Thdr) = 1 then
                  case TPCBs (PCB).State is
                     ...
                     when Fin_Wait_1 =>
                        ...
                        --  This would be the case of receiving FIN only
   ...
   begin
      ...
      --  Verify TCP checksum
      if not NIF.Offload (Netif, NIF.TCP_CS) then
         Buffers.Buffer_Alloc
   ...
   procedure TCP_Fast_Timer (Id : Integer) with
   ...
   procedure TCP_Slow_Timer (Id : Integer) with
